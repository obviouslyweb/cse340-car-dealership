...
[Visit the Render site for the project here.](https://fuse-automobile.onrender.com/) **NOTE:** The Render page no longer works as the database was taken down by the original host (it was a free service only for active students at my university). Until I find a new host, the website will be down but you can still explore the code in the meantime. My apologies for the inconvenience.
...
* Despite showing the "Delete user" button for each user on the Review Accounts page, it is impossible to delete your own account on the page. While this is intended functionality, the button ideally should be hidden on accounts that cannot be deleted in that manner.
